...
class Star(BaseModel):
    id: str = Field(..., examples=["star-001"])
    name: str = Field(..., examples=["Betelgeuse"])
    spectral_type: str = Field(..., examples=["M1-2Ia-ab"])
    distance_light_years: float = Field(..., ge=0, examples=[548.0])
    apparent_magnitude: float = Field(..., examples=[0.42])
...
class StarSearchRequest(BaseModel):
    name: Optional[str] = Field(default=None, examples=["be"])
    spectral_type: Optional[str] = Field(default=None, examples=["M1-2Ia-ab"])
